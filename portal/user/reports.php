                ...
                url: basePathUser + apiUrlGetCoin + '/' + gatewayId,
                    ...
                    success: function(data, textStatus, xhr) {
                        ...
                        var sensor_li_html = '<h5 style="margin-left: 10px;"><strong>Select Sensor Type: <span style="color: red">*</span></strong></h5>' +
                                            ...
                                            '<div class="col-sm-3">' +
                                                '<div class="funkyradio"><div class="funkyradio-success">' +
                                                '<input type="checkbox" value="12" id="txtSensorType6" name="txtSensorType" />' +
                                                '<label for="txtSensorType6">Accelerometer Stream</label>' +
                                                '</div></div>' +
                                            '</div>';
                        ...
                        var excelType_li_html = '<h5><strong>Export Format Type: <span style="color: red">*</span></strong></h5>' +
                        						'<select name="txtExcelType" id="txtExcelType" class="txtExcelType form-control"><option value="csv">CSV Format</option></select><br>'+
									''; //<option value="xls">XLS Excel Format</option>
...
$(document).on('click', '#download-csv', function(e)
{
    ...
    var res_gw_nickname = getGateway_nickname(gateway_id, uid, apikey);
    console.log('res_gw_nickname reports===' + res_gw_nickname);
    var gateway_name;
    if (res_gw_nickname == '' || res_gw_nickname == null || res_gw_nickname == 'null') {
        gateway_name = gateway_id;
    } else {
	...
	if(uid!= '' && apikey!= '' && SelectedCoinsList != '' && SelectedSensorList != '' && selectedTimeDuration != ''){
        ...
        if(excelType == 'csv'){
        	window.open(server_address+"/sensegiz-dev/portal/user/reportscsvdownloaddata.php?gateway_id=" + gateway_id + "&gateway_name=" + gateway_name + "&device_id=" + SelectedCoinsList + "&device_type=" + SelectedSensorList + "&date_format=" + date_format + "&temp_unit=" + temp_unit + "&fromDate=" + fromDate + "&toDate=" + toDate + "&selectedTimeDuration=" + selectedTimeDuration + "&zone_offset=" + encodeURIComponent(zone_offset),"_blank");
        }else if(excelType == 'xls'){
        	window.open(server_address+"/sensegiz-dev/portal/user/reportsexceldownloaddata.php?gateway_id=" + gateway_id + "&gateway_name=" + gateway_name + "&device_id=" + SelectedCoinsList + "&device_type=" + SelectedSensorList + "&date_format=" + date_format + "&temp_unit=" + temp_unit + "&fromDate=" + fromDate + "&toDate=" + toDate + "&selectedTimeDuration=" + selectedTimeDuration + "&zone_offset=" + encodeURIComponent(zone_offset),"_blank");
        }
