...
while(true){
    	...
    	$aQuery = "SELECT *"
        	." FROM accstream_dump"
        	." WHERE action = 0 ORDER BY ad_id ASC LIMIT 1";
    	$db->query($aQuery);
    	$row=$db->single();
...
function publishGateway($pTopic,$tMessage){
    ...
    $mqttPub   = new phpMQTT(SERVER_IP, 1883, $clientId, "sensegiz123", "sg12345"); 
    if ($mqttPub->connect()) {
...
function sendMails($emailsArray,$subject,$message){
		require("/var/www/html/sensegiz-dev/library/sendgrid-php/sendgrid-php.php");
							...
							for($x=0;$x<$mailidslength;$x++){
								...
								curl_setopt($ch,CURLOPT_URL,"http://cm.senegiz.com:3000/ses/?message=".$message."&to=".$emailsArray[$x]."&subject=".$subject."");
