...
$db = new ConnectionManager();
	...
	$sQuery = "SELECT *"
		." FROM daily_report WHERE send_report='Y' AND user_id = 5";	
	...
	if(is_array($res) && !empty($res)){
			...
			$sendTime = $value[report_send_time];


			$stm = strtotime($sendTime);
	
			if($stm != ''){
				$m = strtotime(date('H:i', $stm));
			}else{
				$m = strtotime(date('01:00'));
			}


			$n = strtotime(date('H:i'));
...
EOD;
				...
				for($y=0; $y<$totalcoins; $y++){
					...
					if($acc == 'Y'){
						...
						if(is_array($$varName) && !empty($$varName)){
							...
							$pdf->writeHTML('<img src="/var/www/html/sensegiz-dev/sensegiz-mqtt/daily_report/'.$graphimage.'.png" width="600" height="230" />', true, 0, true, 0);
			...
			$filename= $userID . "_TestReport.pdf"; 
			$filelocation = "/var/www/html/sensegiz-dev/sensegiz-mqtt"; 
			...
			//Update new dates in DB

			$uQuery = " UPDATE daily_report SET report_start_time=:starttime,  report_end_time=:endtime, updated_on = DATE_TRUNC('second', NOW())"
				." WHERE user_id=:userID";
			$db->query($uQuery);	
			$db->bind(':userID',$userID);
			$db->bind(':starttime',$newStart);
			$db->bind(':endtime',$newEnd);	
...
function sendMails($userID, $emailsArray, $subject, $message){
	...
	for($x=0;$x<$mailidslength;$x++){
		...
		$to = new SendGrid\Email(null, "$emailsArray[$x]");

		$content = new SendGrid\Content("text/html", $message);
		$mail = new SendGrid\Mail($from, $subject, $to, $content);

		$att1 = new \SendGrid\Attachment();
		$att1->setContent(base64_encode(file_get_contents("/var/www/html/sensegiz-dev/sensegiz-mqtt/$filename")));
		$att1->setType("text/plain");

		$att1->setFilename("DailyReport.pdf");
