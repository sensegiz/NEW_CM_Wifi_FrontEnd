...
while(true){
	$aQuery = "SELECT *"
        ." FROM data_dump"
        ." WHERE (action=0) AND (data NOT LIKE '%CM') ORDER BY id ASC LIMIT 1";
    $db->query($aQuery);
    $row=$db->single();
	...
	if(!empty($data) && strlen($data)==26){

		$ch = split_on($data, 2);
		$getTime = substr($data, 12, 14);
		$ack = substr($data, 14, 2);
		$datavalue = substr($data, 14, 2);
		$isStartCMD = checkStartCmd($data);
		$dataArr = extractData($data);
		if($isStartCMD == true){
			$gateway_id   = $dataArr[0];
			$device_id    = $dataArr[1];
			
			$query = "INSERT INTO pm_start_cmd(gateway_id, device_id, start_time) "
										   . "VALUES (:gatewayId, :device_id, now())";
						...
						$db->query($query);
							...
							// action 20 is for Start Command
		...
		if($ch[0] == 'aa'){
			...
			if(sizeof($dataArr)==5){			
				$gateway_mapping_id   = $dataArr[0];
				$device_id    = $dataArr[1];
				$XData  = $dataArr[2];
				$YData = $dataArr[3];
				$ZData = $dataArr[4];
				
				$tQuery1 = "SELECT gateway_id"
									." FROM gateway_mapping"
									." WHERE gateway_mapping_id = :gateway_mapping_id";
							$db->query($tQuery1);
							$db->bind(':gateway_mapping_id', $gateway_mapping_id);							
							$res = $db->single();
					...
					if(!empty($isGwAuthenticated)){
						...
						if(!empty($isAuthenticatedDevice)){
							...
							foreach( $device_types as $value ) {
								...
								$pubTopic    =  "$gateway_id".'';
								$pubMessage  =  $gateway_id.','.$device_id.','.$device_type.','.$final.','.$isAuthenticatedDevice['nick_name'].','.date('Y-m-d h:i:s');
			...
			$ackarr = extractData($data);
				...
				$gateway_id   = $ackarr[0];
				$device_id    = $ackarr[1];
				$device_type  = $ackarr[3];
				$device_value = $ackarr[4];
				 
				/* Changes: Available Sensor/Firmware Type */
				if($device_type == 33){
				    $available_sensor_type = "'Accelerometer', 'Gyroscope'";
				    $firmware_type = 'Perimeter Fencing';
				}else if($device_type == 34){
				    $available_sensor_type = "'Accelerometer', 'Gyroscope', 'Temperature', 'Humidity', 'Temperature Stream', 'Humidity Stream'";
				    $firmware_type = 'Threshold Stream';
				}else if($device_type == 35){
				    $available_sensor_type = "'Temperature', 'Humidity', 'Temperature Stream', 'Humidity Stream', 'Accelerometer Stream'";
				    $firmware_type = 'Machine Monitoring';
				}else if($device_type == 36){
				    $available_sensor_type = "'Predictive Maintenance'";
				    $firmware_type = 'Predictive Maintenance';
				}else if($device_type == 37){
				    $available_sensor_type = "'Temperature', 'Temperature Stream'";
				    $firmware_type = 'Surface Temperature';
				}else if($device_type == 38){
					$available_sensor_type = "'Predictive Maintenance'";
					$firmware_type = 'Predictive Maintenance W.R.T XYZ';
				}

				if($device_type == 33 || $device_type == 34 || $device_type == 35 || $device_type == 36 || $device_type == 37 || $device_type == 38){
				                            
				    $sQuery = " UPDATE device_settings SET sensor_active='Y', updated_on=DATE_TRUNC('second', NOW())"
				        . " WHERE gateway_id=:gateway_id AND device_id = :device_id AND (device_sensor IN (".$available_sensor_type.")) AND is_deleted =0";
				    $db->query($sQuery);
				    $db->bind(':gateway_id', $gateway_id);
				    $db->bind(':device_id', $device_id);
				    $db->execute();

				    $sQuery = " UPDATE device_settings SET sensor_active='N', updated_on=DATE_TRUNC('second', NOW())"
				        . " WHERE gateway_id=:gateway_id AND device_id = :device_id AND (device_sensor NOT IN (".$available_sensor_type.")) AND is_deleted =0";
				    $db->query($sQuery);
				    $db->bind(':gateway_id', $gateway_id);
				    $db->bind(':device_id', $device_id);
					$db->execute();
					...
					if($device_type == 36){
						$xyz_combination = 07;
						$sQuery = " UPDATE device_settings SET xyz_combination=:xyz_combination, sensor_active='Y', updated_on=DATE_TRUNC('second', NOW())"
						. " WHERE gateway_id=:gateway_id AND device_id = :device_id AND (device_sensor IN (".$available_sensor_type.")) AND is_deleted =0";
					$db->query($sQuery);
					$db->bind(':xyz_combination', $xyz_combination);
					$db->bind(':gateway_id', $gateway_id);
					$db->bind(':device_id', $device_id);
					$db->execute();
				}
				...
				if($device_type == 38){
					$sQuery = " UPDATE device_settings SET xyz_combination=:device_value, sensor_active='Y', updated_on=DATE_TRUNC('second', NOW())"
					. " WHERE gateway_id=:gateway_id AND device_id = :device_id AND (device_sensor IN (".$available_sensor_type.")) AND is_deleted =0";
				$db->query($sQuery);
				...
				$db->execute();
				    ...
				    $sQuery = " UPDATE gateway_devices SET firmware_type=:firmware_type"
				                                . " WHERE gateway_id=:gateway_id AND device_id = :device_id";
				                            $db->query($sQuery);
				                            $db->bind(':gateway_id', $gateway_id);
				                            $db->bind(':device_id', $device_id);
				                            $db->bind(':firmware_type',$firmware_type);
				                            $db->execute();
				...
				/* Changes Ends: Available Sensor/Firmware Type */


				$rQuery = "SELECT * FROM request_list"
				." WHERE action=0 AND gateway_id=:gateway_id AND device_id =:device_id AND device_type = :device_type"
				." AND request_type IN ('SET', 'SETS', 'EnD', 'DETP', 'FREQ') ORDER BY rl_id ASC LIMIT 1";
				$db->query($rQuery);
				$db->bind(':gateway_id',$gateway_id);
				$db->bind(':device_id',$device_id);
				$db->bind(':device_type',$device_type);	
			...
			if(!empty($rl_id)){	
				...
				if($req_type == 'SET' && $threshold == $device_value){					
							
					$uQuery1 = " UPDATE devices SET threshold_value=:threshold, updated_on = DATE_TRUNC('second', NOW())"
							." WHERE gateway_id=:gateway_id AND device_id =:device_id AND device_type = :device_type AND is_deleted =0 ";
					$db->query($uQuery1);	
					$db->bind(':gateway_id',$gateway_id);
					$db->bind(':device_id',$device_id);
					...
					$sQuery1 = " UPDATE request_list "
							. " SET action = 1, updated_on=:updated_on"
							. " WHERE rl_id=:rl_id";
							$db->query($sQuery1);                    
							$db->bind(':rl_id',$rl_id);               
							$db->bind(':updated_on', $updated_on);							
				...
				if($req_type == 'SETS' && $rate_value == $device_value){
					
					$uQuery1 = " UPDATE devices SET format=:format, rate_value=:rate_value, updated_on = DATE_TRUNC('second', NOW())"
							." WHERE gateway_id=:gateway_id AND device_id =:device_id AND device_type = :device_type AND is_deleted =0 ";
					$db->query($uQuery1);	
					$db->bind(':gateway_id',$gateway_id);
					$db->bind(':device_id',$device_id);
				...
				if($req_type == 'EnD' && $rate_value == $device_value){
					...
					$sQuery = " UPDATE device_settings SET sensor_active=:sensor_active, updated_on=DATE_TRUNC('second', NOW())"
				...
				if($req_type == 'FREQ' && $rate_value == $device_value){
					...
					$uQuery1 = " UPDATE gateway_devices SET frequency=:rate_value, updated_on = DATE_TRUNC('second', NOW())"
				...
				$pubTopic    =  "$gateway_id".'';
				$pubMessage  =  $gateway_id.','.$device_id.','.$device_type.','.$device_value.','.$isAuthenticatedDevice['nick_name'].','.$updated_on.','.$req_type.','.$format.','.$rate_value;
				if($pubTopic!='' && strlen($pubMessage)>12){
					publishGateway($pubTopic,$pubMessage);
				}
		...
		elseif($getTime == '00111111111111'){
			...
			$tquery = "SELECT to_char(current_timestamp, 'HH24:MI:SS')";
			$db->query($tquery);
			$res=$db->resultset();
			...
			$device_value = $hh . $mm . $ss;

			
			//Publish on mQTT directly -- topic 'settime'

			$GatewayMacID = getGatewayMacID($db, $gatewayId);
		...
		if(sizeof($dataArr)==8){
			...
			if($device_type!='12' && $device_type!='14' && $device_type!='15') { // Added for Accelerometer Stream
				...
				if(!empty($isAuthenticatedDevice)){
                    ...
                    $firmware_DD = hexdec($dataArr[3]);
                    $firmware_MM = hexdec($dataArr[4]);
                    $firmware_hh = hexdec($dataArr[5]);
                    $firmware_mm = hexdec($dataArr[6]);
                    $firmware_ss = hexdec($dataArr[7]);
                    
                    if($device_type == 33 || $device_type == 34 || $device_type == 35 || $device_type == 36 || $device_type == 37){
                        ...
                        if($firmware_DD !=00 && $firmware_MM !=00 ){
                            $c_year = date('Y');
                            $last_firmware_update = $c_year.'-'.$firmware_MM.'-'.$firmware_DD.' '.$firmware_hh.':'.$firmware_mm.':'.$firmware_ss;
                            $sQuery = " UPDATE gateway_devices SET firmware_release_date='$last_firmware_update'"
                                . " WHERE gateway_id=:gateway_id AND device_id = :device_id";
                            $db->query($sQuery);
                            $db->bind(':gateway_id', $gateway_id);
                            $db->bind(':device_id', $device_id);
